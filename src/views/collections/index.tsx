...
export interface CollectionFilterMetadata {
  name: string
  values: { [key: string]: { total: number; floor: number } }
}
...
export const CollectionsView = () => {
  ...
  >([])
  const [filtersData, setFiltersData] = useState<CollectionFilterMetadata[]>([])
  ...
  const { data, refetch, loading } = useCollectionFiltersQuery({
    variables: { collection: formatToSpaces(collectionName) },
  })
  ...
  const formatMetaData = (data: any): any => {
    if (data) {
      const formatted = Object.keys(data).map(function (key) {
        return { name: key, values: data[key] }
      })
      return formatted
    }
    return {}
  ...
  useEffect(() => {
    if (data?.collectionFilters) {
      //@ts-ignore
      const filtered = data.collectionFilters.nodes[0]

      // setFiltersData(JSON.parse(filtered?.traits))
      //@ts-ignore
      const traits = filtered ? JSON.parse(filtered?.traits) : {}
      setFiltersData(formatMetaData(traits))
    }
  }, [data, collectionName])
      ...
      const accountsDecoded = (
        await Promise.all(
          response?.offers.map(async (offer: any) => {
            ...
            } else if (!!offer && offer.contract == DUTCH_AUCTION_CONTRACT_ID) {
              const dutchOfferInfo = await dutchOfferRes(offer.mint)

              const priceDecremented =
                dutchOfferInfo.startingPrice -
                Math.floor(
                  (Math.floor(Date.now() / 1000) -
                    dutchOfferInfo.startingTimestamp) /
                    dutchOfferInfo.interval
                ) *
                  dutchOfferInfo.priceStep
              return {
                mint: dutchOfferInfo.mint,
                price:
                  (priceDecremented > dutchOfferInfo.reservedPrice
                    ? priceDecremented
                    : dutchOfferInfo.reservedPrice) / LAMPORTS_PER_SOL,
                escrowPubkeyStr: offer.pk,
                owner: dutchOfferInfo.owner,
                uri: offer.uri,
                collectionName: offer.collection,
                contract: dutchOfferInfo.contract,
                isVerifeyed: offer.verifeyed,
                metadata: offer.metadata,
                lastPrice: offer.lastPrice,
                startingPrice: dutchOfferInfo.startingPrice,
              } as ActiveOffer
            }
            return false
          })
  ...
  const dutchOfferRes = async (mint: string) => {
    try {
  ...
  useEffect(() => {
    const highestSaleEndpoint = `${BASE_URL_HIGHEST_SALE_RETRIEVER}?collection=${formatToSpaces(
      collectionName
    )}`
    ...
    const collectionSalesHistoryEndpoint = `${BASE_URL_COLLECTION_SALES_HISTORY}?${COLLECTION_SALES_HISTORY_PARAM}=${formatToSpaces(
      collectionName
    )}&${TYPE_SALES_HISTORY_PARAM}=SALE`
  ...
  if (
    ...
    setTimeout(() => {
      if (redirectCount === 0) {
        setRedirectCount(1)
        push(`${ROUTES.COLLECTIONS}/${formatToSpaces(collectionName)}`)
      }
    }, 1000)
    if (redirectCount === 1) {
      return <NotFoundView />
