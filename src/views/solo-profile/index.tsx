...
export const SoloProfileView = () => {
  ...
  const [editListed,setEditListed] = useState<boolean>(false)
  ...
  const [isLoadingListed,setIsLoadingListed] = useState<boolean>(false)
  ...
  const getListedNfts = useCallback(async () => {
    ...
    console.log('listed',listedNftsFromDirectFiltered)
...
const saveChangesListed= async (items:any) => {
  setIsLoadingMasonry(true);
  setEditListed(false);
  try{
     await authApiRequest(SOLO_PROF_GALLERY, { mints:items.toString() }, "POST", "application/json", wallet)
            ...
            >
              ...
              {!!publicKey &&
                      ...
                      {!followed && (
                        ...
                        >
                          ...
                          <Menu.Items className="z-40 flex flex-col origin-top-right absolute right-0 p-2 bg-gray-900 rounded-lg w-auto  overflow-y-auto">
                            ...
                            <Menu.Item>
                            <button
                            onClick={goToEditListed}
                              className="z-90 px-4 py-2 hover:text-gray-300 rounded-md hover:bg-gray-600 font-light flex justify-between"
                            >
                            <p
                            className="text-xs md:text-sm"> Listings </p>
                            </button>
                            </Menu.Item>
                            </Menu.Items>
                          </Transition>
                ...
                <TabPanel {...tab}>
                  <>{listedNfts &&
                    <CreationsMasonry
                      arrayDisplay={listedNfts}
                      arrayEdit={listedNfts}
                      edit={editListed}
                      theme={profileTheme}
                      cancelEdit={cancelEditListed}
                      saveChanges={saveChangesListed}
                      isLoading={isLoadingMasonry}
                      isOwner={publicKey?.toString() == artist?.wallet_key.toString()}/>
                      }
                        </>
