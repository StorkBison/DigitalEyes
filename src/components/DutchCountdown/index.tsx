...
export const DutchCountdown : React.FC<DutchCountdownProps> = ({ timeLeft,refreshItem }) => {
  ...
  return (
    <div className="flex">
      ...
      </div><p className="text-xs text-gray-600 mt-4">days</p>
      <div className={timeContainerClass}>
        <p className="m-auto">{time?.hours}</p>
      </div><p className="text-xs text-gray-600 mt-4">hours</p>
      <div className={timeContainerClass}>
        <p className="m-auto">{time?.minutes}</p>
      </div><p className="text-xs text-gray-600 mt-4">mins</p>
      <div className={timeContainerClass}>
        <p className="m-auto"> {time?.remainingSeconds}</p>
      </div><p className="text-xs text-gray-600 mt-4">seconds</p>
    </div>
  )
}
