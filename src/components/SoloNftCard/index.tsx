...
export const SoloNftCard: React.FC<SoloNftCardProps> = ({ offer }) => {
  ...
  const [artistImageCacheFailed, setArtistImageCacheFailed] =
  ...
  const goToNFT = (e: any) => {
    return `${ROUTES.SOLO_ITEM}/${offer.mint}`
  }
  ...
  const goToArtist = (e: any) => {
    if (offer?.artistUser) {
      if (Boolean(e.ctrlKey || e.metaKey || e.button === 1)) {
        window.open(
          `${window.location.origin}${ROUTES.SOLOPROFILE}/${offer.artistUser}`
        )
      } else {
  ...
  const getReactionInfo = async () => {
    if (wallet?.publicKey) {
      ...
      const totalVotes: number =
        parseInt(voteCount?.reaction_count.upvotes) -
        parseInt(voteCount?.reaction_count.downvotes)
      setVoteCount(totalVotes)
    }
  }

  const anonDisplay: string | undefined = offer.solo
    ? `Anon${shortenAddress(offer?.collectionName.split("#")[2])}`
    : undefined
    ...
    <div>
      <div className="grid grid-cols-2 flex justify-between py-2 ">
        <div onClick={goToArtist}>
          {offer?.artistImage &&
          isImageInCache(offer?.artistImage) &&
          !artistImageCacheFailed ? (
            <IKImage
              ...
              className={
                offer?.artistUser
                  ? "w-12 h-12 border-1 border-black rounded-full object-cover shadow-2xl inline cursor-pointer"
          ...
          {offer?.artistUser ? (
            <div className="text-white text-lg text-left inline cursor-pointer ml-4">
              {offer?.artistUser}
            </div>
          ) : (
            <div className="text-white text-lg text-left inline opacity-60 ml-4">
              {anonDisplay}
            </div>
          )}
        </div>
      ...
      >
        <Link to={goToNFT}>
          <div className="col-span-2 w-full">
            ...
            isImageInCache(offer.metadata.image) &&
            !cacheFailed ? (
              <IKImage
                urlEndpoint={IMAGE_KIT_ENDPOINT_URL}
      ...
      <div className="col-span-2 py-1 flex justify-end">
        <p className="text-white text-xs text-left inline">
          {offer?.metadata.name}
        </p>
      </div>
      <div className="col-span-2 py-1 flex justify-between items-center">
        ...
        <p
          className={
            voteCount == 0
              ? "text-xs opacity-0 w-10 text-center"
              : "text-xs opacity-70 w-10 text-center"
          }
        >
          {voteCount}
        </p>
