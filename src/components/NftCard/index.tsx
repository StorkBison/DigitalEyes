...
import {
  ...
  Tooltip,
  TooltipArrow,
  TooltipReference } from "reakit/Tooltip";
...
export const NftCard: React.FC<NftCardProps> = ({ offer, wallet,collectionVerifeyed,isDutch }) => {
  ...
  const copyLink = () => {
    const isSolo: boolean = collectionPath.split("#")[0] == "solo"

    setCopied(true)
    navigator.clipboard.writeText(
      `${window.location.origin}${ROUTES.ITEM}${isSolo ? '/':`/${collectionPath}/`}${offer.mint}?pk=${offer.escrowPubkeyStr}${walletParam}`
    )
    setTimeout(() => {
      setCopied(false)
    }, 2000)
  ...
  const goToNFT = (e: any) => {
    ...
    if (collName?.split("#")[0] == "solo") {
      ...
      if (Boolean(e.ctrlKey || e.metaKey || e.button === 1)) {
        ...
        history.push(
          `${ROUTES.ITEM}/${formatToHyphen(collectionPath)}/${offer.mint}?pk=${
            offer.escrowPubkeyStr
          }${walletParam}`
        )
      }
    }
    ...
    >
      <LazyLoad height={200} offset={100}>
        ...
        <div className="flex-initial relative">
          ...
          <div
            className={`px-4 pb-2 flex justify-between items-end ${
              isToken && window.location.pathname.includes("/wallet")
                ? "hidden"
                : ""
            }`}
          >
            <p className="font-bold text-xl -ml-2 leading-none">
              {!isDutch && offer.price ? <span className="mt-1">◎{offer.price}</span> : " "}
              {!!isDutch && offer.price ? <span className="mt-1">~ ◎{(offer.price).toFixed(3)} </span> : " "}
            </p>
            <p className="text-xs text-gray-400">
              {!isDutch && offer.lastPrice && offer.lastPrice > 0 && (
                <span>Last ◎{offer.lastPrice / LAMPORTS_PER_SOL}</span>
              )}
              {!!isDutch && <span className="text-xxs text-gray-600 border border-gray-600 p-1 m-1 rounded-md">DUTCH</span>}
