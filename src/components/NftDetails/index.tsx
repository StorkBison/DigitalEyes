...
import { makeOffer } from '../../contracts/offer'
...
export const NftDetails: React.FC<NftDetailsProps> = ({
  ...
  const [dutchPriceStep, setDutchPriceStep] = useState<number>(0)
  const [timeTilReserve, setTimeTilReserve] = useState<number>(0)
  ...
  const copyLink = () => {
    ...
    navigator.clipboard.writeText(
      `${window.location.origin}${ROUTES.ITEM}${isSolo ? '/':`/${collectionName}/`}${nftData.mint}?pk=${nftData.escrowPubkeyStr}`
    )
    setTimeout(() => {
      setCopied(false)
    }, 2000)
  ...
  useEffect(() => {
    if (!connection || !wallet?.publicKey || !nftData || isTokenAccountAcquired) return
    const getTokenAccount = async () => {
      if (wallet?.publicKey) {
        ...
        if (isToken) {
        ...
        } else if (nftData.contract == DIRECT_SELL_CONTRACT_ID) {
          const tokenAccountInfo = await connection.getParsedTokenAccountsByOwner(
            new PublicKey(nftData.initializerPubkey!),
            {mint:new PublicKey(mint)}
          )

          let nftIsInWallet : boolean = false;
          if (tokenAccountInfo.value.length) {
            tokenAccountInfo.value.forEach((info) => {
              console.log(info.account.data.parsed)
          ...
          if (nftData.initializerPubkey == connectedWallet && !nftIsInWallet) {
            setIsOwnedByWallet(true)
            setNotice(
              "It looks like you have moved this item to a different wallet. Please move it back to unlist or lower the price."
            )
  ...
  useEffect(() => {
    if( (dutchReservePrice != 0 && dutchStartPrice != 0) && dutchStartPrice - dutchReservePrice <= 0 || (dutchReservePrice < 0 || dutchStartPrice <0 ) ) {
      console.log(dutchReservePrice,dutchReservePrice);


      setListingNotice("Values cannot be 0 or less. Starting price must be higher than reserve price.");
    } else {
      setListingNotice("");
    }
  ...
  if(dutchDuration=="custom"){
    if(dutchCustomDurationDays % 1 != 0 || dutchCustomDurationHours % 1 != 0 || dutchCustomDurationDays <0 || dutchCustomDurationHours <0 || (dutchCustomDurationDays==0 && dutchCustomDurationHours==0)) {
      setListingNotice("Invalid Input. No decimals allowed & value must be above 0");
    } else {
      setListingNotice("");
    }}
  ...
  if(dutchDuration=="advanced"){
    ...
    if(dutchDurationCustomIntervalHours % 1 != 0 || dutchDurationCustomIntervalMins % 1 != 0 || dutchDurationCustomIntervalHours < 0 || dutchDurationCustomIntervalMins <0) {
      console.log(dutchDurationCustomIntervalHours,dutchDurationCustomIntervalMins );

      setListingNotice("Invalid Input. No decimals or negative values allowed. Must be greater than 0");
    } else if((priceString.split('.')[1] && priceString.split('.')[1].length > 2 )|| dutchIntervalCustomPrice <0 )
     {
      setListingNotice("Too may decimals or invalid amount in Price Decrement");
    } else if (dutchIntervalCustomPrice > (dutchStartPrice - dutchReservePrice))
    {
      setListingNotice("Your price change per interval does not allow a sale above reserve price");
...
useEffect(() => {
  ...
  useEffect(()=>{
    let priceDiff = dutchStartPrice - dutchReservePrice;
    setDutchPriceStep(
      dutchDuration=="1-day" ? (priceDiff/(1*24*60*60))*300 :
      dutchDuration=="3-day" ? (priceDiff/(3*24*60*60))*600 :
      dutchDuration=="7-day" ? (priceDiff/(7*24*60*60))*1800 :
      dutchDuration=="custom" ? (priceDiff/dutchCustomDuration)*1800 :
      dutchDuration=="advanced" ? dutchIntervalCustomPrice : 0
    )
  },[dutchReservePrice,dutchStartPrice,dutchDuration,dutchCustomDuration,dutchIntervalCustomPrice])
  ...
  async function buyItemDutch(): Promise<void> {
    ...
    try {
      if (wallet?.publicKey && nftData.saleInfo) {
        ...
        setNotificationTitle(`Success!`)
        setNotificationDesc(`You've successfully bought ${name} for ${price} SOL. Please allow some time for the changes to be reflected in the listing and in your wallet.`)
        setNotificationCanClose(true)
        setModalTimer(5)
      }
    } catch (error) {
  ...
  async function makeOfferFunction(): Promise<void> {
    setButtonLoading(true)
    try {
      if (wallet) {
        // console.log(connection, wallet, offerAmount, mint)
        let res = await makeOffer(connection, wallet, offerAmount, new PublicKey(mint), expireReturn())

        if (res == true) {
          setNotificationTitle(`Success!`)
          setNotificationDesc(
            `You've successfully made offer to buy ${name} for ${offerAmount} SOL. Please allow some time for the changes to be reflected in the listing and in your wallet.`
          )
          setNotificationCanClose(true)
          setModalTimer(5)
        } else {
          setNotificationTitle(`Oops, something went wrong!`)
          setNotificationDesc("Please deposit first.")
          setNotificationCanClose(true)
        }
          ...
          <div className="col-span-1 md:col-span-3">
              ...
              <div className="grid grid-cols-2">
                ...
                {nftData.reservedPrice && nftData.reservedPrice/LAMPORTS_PER_SOL != nftData.price && (
                  <div className="col-span-2">
                    ...
                    />
                    <p className="text-base text-gray-500 uppercase my-2">Next Price: {nftData.price-(nftData.priceStep/LAMPORTS_PER_SOL)}   ◎</p>
                ...
                {isOwnedByWallet && nftData.endingTs && nftData.reservedPrice && nftData.reservedPrice/LAMPORTS_PER_SOL != nftData.price && (
                  <div className="col-span-2">

                    <p className="text-white font-light inline">Dutch Auction on this item will reach reserve price of ◎ {nftData.reservedPrice/LAMPORTS_PER_SOL} on {dayjs.unix(nftData.endingTs).format("DD MMM YYYY HH:ss")} approx.</p>
                    <div className="mb-6">
            ...
            <div className="">
              ...
              <div className="">
                ...
                {nftData && !isTokenWithoutPK && (
                  <div className="flex space-x-2">
                    ...
                    >
                      ...
                      {offerAmount > 0
                        ? `Make an offer ${offerAmount}`
                        : `Enter Lower Price`}
                    </button>
                    Offer Amount: <input value={offerAmount} style={{color: "black"}} onChange={(e: any) => {setOfferAmount(e.target.value)}}/>
                    Offer Expire Time : 
                ...
                {!isListed && isOwnedByWallet && statusOnchain != "listed" && statusOnchain != "listedOffer" && (
                  <div
                    ...
                    <TabList {...tab} aria-label="My tabs">
                      ...
                      <Tab
                        ...
                        style={{
                          ...
                          padding: "1rem",
                          borderRadius: "5px 5px 0 0",
                          backgroundImage: "linear-gradient(45deg, #27272a 25%, #52525b 50%, #27272a 75%)",
                          backgroundSize: "400% 400%",
                          animation: "shimmer 6s linear infinite",
                        }}
                      >
                        Dutch Auction
                        <p className="absolute top-0 right-1 text-gray-400 text-xxs p-1">BETA</p>
                      </Tab>
                    </TabList>
                    <TabPanel
                      {...tab}
                        ...
                        <div className="mb-5">
                          <div className="grid grid-cols-2 mb-5 gap-x-5">
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                id="price"
                                className="block w-full border border-gray-600 bg-transparent rounded-md text-gray-500 focus:text-white px-4 py-3 focus:outline-none font-light"
                                placeholder="Lowest amount accepted"
                                aria-describedby="price-currency"
                                onWheel={(e) => (e.target as HTMLElement).blur()}
                              />
                            </div>
                          ...
                          { (dutchDuration == 'custom' && (dutchCustomDurationDays > 0 || dutchCustomDurationHours > 0)) && listingNotice=='' &&
                              <small className="mb-5 block">
                                This auction will last {dutchCustomDurationDays} day(s){ dutchCustomDurationHours> 0 ? ` and ${dutchCustomDurationHours} hour(s).`:`.`}
                              </small>
                          }
                          ...
                          { dutchDuration == 'advanced' && (
                            <>
                              ...
                              { (dutchIntervalCustomPrice > 0 && (dutchDurationCustomIntervalHours >= 0 && dutchDurationCustomIntervalMins >= 0)) && (
                                <small className="mb-5 block">
                                  The price will lower by ◎{dutchIntervalCustomPrice} every {dutchDurationCustomIntervalHours} hour(s) and {dutchDurationCustomIntervalMins} minute(s).
                                   The auction will last approx. {((((dutchStartPrice - dutchReservePrice)/dutchIntervalCustomPrice)*dutchDurationCustomInterval)/(60*60))} hours.
                                </small>
                              )}

                            </>
                          )}
                          { (dutchDuration == '1-day' || dutchDuration == '3-day' || dutchDuration == '7-day' || dutchDuration == 'custom') &&
                            <small>
                            The price will lower by ◎{dutchPriceStep} every {dutchDuration=="1-day" ? "5" : dutchDuration=="3-day" ? "10" : (dutchDuration=="7-day" || dutchDuration=="custom") ?  "30" : "0"} minutes.
